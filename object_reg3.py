...
def main(_argv):
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            print("x_axis/y_axis {}".format((x_axis/y_axis)))
            #if(int(track.track_id)>=3 or (int(track.track_id)>10 and int(track.track_id)<20 ) ):

            if(class_name=='entrance'):
                print("test2")
                blk = np.zeros(frame.shape, np.uint8)
                cv2.rectangle(blk,(int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (255, 0, 0), cv2.FILLED)
                frame =cv2.addWeighted(frame, 1.0, blk, 0.5, 1)
            cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color, 2)
            cv2.rectangle(frame, (int(bbox[0]), int(bbox[1]-30)), (int(bbox[0])+(len(class_name)+len(str(track.track_id)))*17, int(bbox[1])), color, -1)
            cv2.putText(frame, class_name + "-" + str(track.track_id),(int(bbox[0]), int(bbox[1]-10)),0, 0.75, (255,255,255),2)
